...
export class Pts {
  constructor(private _program: PtsProgram) {}
  ...
  public batchTransfer(from: ActorId, to_ids: Array<ActorId>, amounts: Array<number | string | bigint>): TransactionBuilder<null> {
    if (!this._program.programId) throw new Error('Program ID is not set');
    return new TransactionBuilder<null>(
      this._program.api,
      this._program.registry,
  ...
  public async accrual(originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<bigint> {
  ...
  public async getBalance(id: ActorId, originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<bigint> {
  ...
  public async getRemainingTimeMs(id: ActorId, originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<number | string | bigint | null> {
  ...
  public async timeMsBetweenBalanceReceipt(originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<bigint> {
  ...
  public subscribeToBatchTransferedEvent(callback: (data: { from: ActorId; to_ids: Array<ActorId>; amounts: Array<number | string | bigint> }) => void | Promise<void>): Promise<() => void> {
    return this._program.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      if (!message.source.eq(this._program.programId) || !message.destination.eq(ZERO_ADDRESS)) {
        return;
      }
      ...
      if (getServiceNamePrefix(payload) === 'Pts' && getFnNamePrefix(payload) === 'BatchTransfered') {
        callback(this._program.registry.createType('(String, String, {"from":"[u8;32]","to_ids":"Vec<[u8;32]>","amounts":"Vec<u128>"})', message.payload)[2].toJSON() as unknown as { from: ActorId; to_ids: Array<ActorId>; amounts: Array<number | string | bigint> });
      }
    });
  }
}