...
function bandersnatchAdd(F, a, d, X1, Y1, Z1, X2, Y2, Z2) {
    const A = F.mul(X1, X2);
    const B = F.mul(Y1, Y2);
    const C = F.mul(Z1, Z2);
...
describe("BLS12-381 Point Check", function () {
    ...
    it("Should accept valid public key on BLS12-381", async () => {
        const circuit = await circom_tester.wasm(CIRCUIT_PATH, { prime: "bls12381" });
...
function projectiveAdd(F, a, d, X1, Y1, Z1, X2, Y2, Z2) {
    ...
    const X3 = F.mul(X, F.sub(F.mul(F.add(X1, Y1), F.add(X2, Y2)), F.add(C, D)));
...
function scalarMulJS(F, a, d, X, Y, Z, n) {
    if (n === 0n) {
