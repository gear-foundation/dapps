...
function getPkZk(pub) {
    const { x, y, z } = ecPointToNumberArrays(pub);
    return { x, y, z };
}
async function loadAccounts(paths, pass) {
...
async function main() {
    ...
    const aggKey = players.reduce((acc, p) => projectiveAdd(F, a, d, acc, p.pk), { X: 0n, Y: 1n, Z: 1n });
    let deck = initDeck(numCards);
