...
use crate::services::session::{
    ActionsForSession, Event, SessionData, SessionError, SessionMap, SignatureData,
};
use gstd::{exec, msg};
...
pub fn create_session(
    ...
    if duration < config.minimum_session_duration_ms {
    ...
    let number_of_blocks = u32::try_from(duration.div_ceil(config.s_per_block * 1_000))
    ...
    let account = match signature {
        Some(sig_bytes) => {
            check_if_session_exists(sessions, &key)?;
            ...
            sessions.entry(key).insert(SessionData {
                key: msg_source,
                expires,
                allowed_actions,
