use dutch_auction_io::auction::{Action, CreateConfig, Duration, Error, Event};
...
use non_fungible_token_io::{Constraints, InitNFT, NFTEvent};
...
pub fn init(sys: &System) -> Program {
    ...
    assert!(result.contains(&(
        owner_user,
        Ok::<dutch_auction_io::auction::Event, Error>(Event::AuctionStarted {
...
pub fn init_nft(sys: &System, owner: u64) {
    ...
    let res = nft_program.send(
        owner,
        non_fungible_token_io::NFTAction::Mint {
    ...
    let res = nft_program.send(
        owner,
        non_fungible_token_io::NFTAction::Approve {
            to: 1.into(),
            token_id: 0.into(),
            transaction_id: 1u64,
        },
    ...
    let log = Log::builder()
        .dest(owner)
        .payload(non_fungible_token_io::NFTEvent::Approval(approval));
    assert!(!res.main_failed());
...
pub fn nft_owner(nft_program: &Program, from: u64, token_id: TokenId) -> RunResult {
    nft_program.send(from, non_fungible_token_io::NFTAction::Owner { token_id })
}
