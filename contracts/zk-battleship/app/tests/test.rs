...
fn start_game() {
    ...
    // let x = (11_u8, StepResult::Injured, None::<ActorId>).encode();
    // println!("\n x {:?}", x);
    // let x = (11, StepResult::Injured, None::<ActorId>).encode();
    // println!("\n x {:?}", x);
    // let encoded_request = ["Single".encode(), "ConsolidateMove".encode(), (11_u8, StepResult::Injured, None::<ActorId>).encode()].concat();
    // let res = battleship.send_bytes(USERS[0], encoded_request);
