...
export function useInitBattleData() {
  ...
  useEffect(() => {
    if (window) {
      ;(window as any).BattleAddress = process.env.REACT_APP_GAME_ADDRESS
    }
  }, [])
  ...
  useEffect(() => {
    if (state) {
      ...
      if (prevBattleState.current !== state.state)
        prevBattleState.current = state.state
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [navigate, state])
