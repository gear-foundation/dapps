...
export async function deploy(name: string, symbol: string, base_uri: string) {
  ...
  const gas = await gearApi.program.gasSpent.init(`0x${account.address}`, code, initMTK, 0, meta);
...
export async function send(payload: any, destination: Buffer | `0x${string}`) {
  ...
  const gas = await gearApi.program.gasSpent.handle(`0x${account.address}`, destination, payload, 10010, meta);
