...
export const BattleRoundPlayers = () => {
  ...
  useEffect(() => {
    if (battle && account && currentPlayer) {
      setIsAllowed(
        battle.players[currentPlayer].owner === account.decodedAddress
      )
    }
  ...
  const onAttack = () => {
    setIsPending(true)
    handleMessage(
      { MakeMove: { pair_id: currentPairIdx, tmg_move: { Attack: null } } },
      { onSuccess, onError }
    )
  }
  const onDefence = () => {
    setIsPending(true)
    handleMessage(
      { MakeMove: { pair_id: currentPairIdx, tmg_move: { Defence: null } } },
      { onSuccess, onError }
    )
  }
        ...
        <div className="relative grow grid grid-cols-[40%_40%] justify-between gap-10 mt-10 xxl:mt-15">
          <div className={cnWrapper}>
            <TamagotchiAvatar
              ...
              isWinner={
                battle.state === 'WaitNextRound' &&
                battle.pairs[currentPairIdx].winner === rivals[0].tmgId
              }
              isDead={!toNumber(rivals[0].health)}
              damage={roundDamage ? Math.round(+roundDamage[1] / 25) : 0}
              action={
                roundDamage &&
                (roundDamage[3] === null ? 'Skipped' : roundDamage[3])
              }
              asPlayer
            />
          </div>
              ...
              {!battle.pairs[currentPairIdx].gameIsOver ? (
                ...
                <p className="text-center text-2xl leading-normal xxl:typo-h2 truncate max-w-[13ch] font-bold">
                  <span className="text-primary">
                    {battle.players[battle.pairs[currentPairIdx].winner].name}
                  </span>
                  <br />
                  Winner
                </p>
          ...
          <div className={cnWrapper}>
            <TamagotchiAvatar
              ...
              isWinner={
                battle.state === 'WaitNextRound' &&
                battle.pairs[currentPairIdx].winner === rivals[1].tmgId
              }
              isDead={!toNumber(rivals[1].health)}
              damage={roundDamage ? Math.round(+roundDamage[2] / 25) : 0}
              action={
                roundDamage &&
                (roundDamage[4] === null ? 'Skipped' : roundDamage[4])
              }
              reverse
              asPlayer
            />
