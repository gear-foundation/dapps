...
class GameEngine {
  ...
  constructor(canvas: HTMLCanvasElement, gameActions: GameActions, timer: number, level: IGameLevel) {
    this.canvas = canvas;
  ...
  drawGameEnd() {
    const isCollideWith = this.enemies.some((enemy) => enemy.collideWith(this.character));
