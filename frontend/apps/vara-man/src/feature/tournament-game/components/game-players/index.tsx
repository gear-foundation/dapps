...
export const GamePlayers = () => {
	...
	const startTime = parseInt(
		tournamentGame?.[0].stage.Started.replace(/,/g, '') || '0',
		10
	)
	const durationMs = parseInt(
		tournamentGame?.[0].durationMs.replace(/,/g, '') || '0',
		10
	)
	...
	const bid =
		parseFloat(String(tournamentGame?.[0].bid).replace(/,/g, '') || '0') /
		10 ** decimals
				...
				{sortedParticipants?.map(
					(
						...
						const isActivePlayer =
							account?.decodedAddress === participant.address
						...
						return (
							...
							>
								...
								>
									{index + 1}
									{index === 0 && (
										<SpriteIcon name="medal-fill" height={24} width={24} />
									)}
									{(index === 1 || index === 2) && (
										<SpriteIcon name="medal-line" height={24} width={24} />
									)}
								</div>
