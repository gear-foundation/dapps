...
function placeShip(board: Entity[], size: number, row: number, col: number, isVertical: boolean): number[] {
...
function occupyCells(board: Entity[], row: number, col: number): void {
  for (let i = row - 1; i <= row + 1; i++) {
    for (let j = col - 1; j <= col + 1; j++) {
      if (i >= 0 && i < Math.sqrt(board.length) && j >= 0 && j < Math.sqrt(board.length)) {
...
function generateShipsField(rows: number, cols: number): number[][] {
  ...
  for (const size of shipLengths) {
    ...
    for (let attempts = 0; attempts < 100; attempts++) {
      ...
      if (checkEmptyField(board, size, Math.floor(position / cols), position % cols, isVertical)) {
        ships.push(placeShip(board, size, Math.floor(position / cols), position % cols, isVertical));
        shipPlaced = true;
        break;
      }
    }
