...
export const TamagotchiAvatar = ({
  ...
  return (
    <div className={cn('relative text-[#1852FF]', className ?? 'grow w-full h-30 aspect-square')}>
      ...
      {itemsUsed?.includes('bag') && <SpriteIcon name="body-bag" section={s} className={cx} />}
      ...
      {!tamagotchiDied && glasses && <SpriteIcon name={glasses} section={s} className={cx} />}
      {!tamagotchiDied && itemsUsed?.includes('hat') && <SpriteIcon name="head-hat" section={s} className={cx} />}
      ...
      {Boolean(damage) && (
        <div className="absolute top-1/4 right-15 w-12 h-12 grid place-items-center">
          <SpriteIcon name="damage" section={s} className="absolute inset-0 w-full h-full" />
