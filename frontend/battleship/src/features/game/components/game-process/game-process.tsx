...
export default function GameProcess() {
  ...
  const totalShips = gameState?.botShips.reduce(
    (total, [shipType, shipCount]) => {
      return total + parseInt(shipCount, 10)
    },
    0
  )
  const totalShoots = gameState ? parseInt(gameState.totalShots) : 0
  const successfulShoots =
    gameState?.botBoard.filter((x) => x === 'DeadShip' || x === 'BoomShip')
      .length ?? 0
  const efficiency =
    totalShoots !== 0 ? ((successfulShoots / totalShoots) * 100).toFixed(2) : 0
