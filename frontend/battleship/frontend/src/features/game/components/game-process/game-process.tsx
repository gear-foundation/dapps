        ...
        <div className={styles.container}>
            <div className={styles.content}>
                ...
                <div className={styles.gameInfo}>
                    <Text size="sm" weight="normal">Time: <span>{elapsedTime}</span ></Text>
                    <Text size="sm" weight="normal">Total shots: <span>{totalShoots}</span></Text>
                    <Text size="sm" weight="normal">Successful hits: <span>{successfulShoots}</span></Text>
                    <Text size="sm" weight="normal">Efficiency: <span>{efficiency}%</span></Text>
                </div>
            </div>
            <div className={styles.enemyShips}>
                <Text size="sm" weight="normal" className={styles.text}>Enemy Ships: {totalShips}</Text>

                <div className={styles.listShips}>
                    {gameState?.botShips.map(([shipType, shipCount], index) => {
                        const numberOfBlocks = parseInt(shipType, 10);
                        const shipsToRender = Array.from({ length: parseInt(shipCount, 10) });
