...
const DialogHeader = ({
  ...
  <div
    className={clsx(
      'flex flex-col space-y-2 rounded-lg text-center sm:text-left',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={clsx('text-2xl font-bold leading-8', className)}
    {...props}
  />
))
...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Content ref={ref} {...props} asChild>
    <m.div
      ...
      exit="exit"
      className={clsx(
        'fixed z-50 py-7.5 px-8 w-full bg-[#F6F8F8] shadow-xl rounded',
        className ?? 'max-w-md'
      )}
    >
      {children}
